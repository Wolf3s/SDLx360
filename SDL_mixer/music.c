...
int open_music(SDL_AudioSpec *mixer)
{
	...
	if ( !native_midi_ok ) {
		native_midi_ok = (SDL_getenv("SDL_NATIVE_MUSIC") != NULL);
	}
	if ( native_midi_ok )
#endif
...
{
	const char* force = SDL_getenv("SDL_FORCE_SOUNDFONTS");
...
#if defined(__MINGW32__) || defined(__MINGW64__) || defined(_XBOX)
